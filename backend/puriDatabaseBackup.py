...
class imageTagEntry(baseDatabaseClass):
    __tablename__ = 'imageTagTable'

    imageTagId = Column(Integer, Sequence('imageTagId'), primary_key=True)
...
class database:
    ...
    def addPixivImageToDatabase(self, message):
        ...
        if imageId == 0:
            ...
            # Add the imageTags to the database
            imageId = self.addImageTagEntry(0, tagIds[0])
            for i in range(1, len(tagIds)):
                self.addImageTagEntry(imageId, tagIds[i])
    ...
    def isFileInDatabase(self, imageHash):
        ...
        imageTag = self.session.query(imageTagEntry).\
    ...
    def findImageTagEntryId(self, imageId, tagId):
        imageTag = self.session.query(imageTagEntry).\
        ...
        if imageTag == None:
            return -1
        elif imageTag.imageId == 0:
            imageTag.imageId = imageTag.imageTagId
            #self.session.commit()

        return imageTag.imageId


    def addImageTagEntry(self, imageId, tagId):
        imageTag = imageTagEntry(imageId=imageId, tagId=tagId)
        self.session.add(imageTag)
        #self.session.commit()
        imageId = self.findImageTagEntryId(imageId, tagId)
        return imageId
    ...
    def get_image_info(self, imageId):
        imageTags = self.session.query(imageTagEntry).\
                        filter_by(imageId=imageId).\
                        all()
        ...
        for imageTag in imageTags:
            tag = self.session.query(tagEntry).\
                            filter_by(tagId=imageTag.tagId).\
                            first()
    ...
    def localImageSearchByTags(self, message):
        ...
        searchTags = [(None, x[0]) if len(x) == 1 else (x[0], x[1]) for x in searchTags]
        ...
        for searchTag in searchTags:
            query = self.session.query(tagEntry).\
                            filter_by(tagValue=searchTag[1])
            if searchTag[0] != None:
                query = query.filter_by(tagAttribute=searchTag[0])
        ...
        queryResults = []
        for tagId in tagIds:
            queryResult = self.session.query(imageTagEntry).\
                            filter_by(tagId=tagId).\
                            all()
            if queryResult != None:
